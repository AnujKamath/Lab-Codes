...
int isConnected(int a,int b,int c)
{
    if(t[a][b]!=0) return 1;
    for(int i=1;i<=n;i++)
    {
...
void kruskals()
{
    while(e && k<n-1)
    {
        ...
        a[ivalue][jvalue]=0;
        a[jvalue][ivalue]=0;
        if(check_reach(ivalue)==1&&check_reach(jvalue)==1)
        {
            if(isConnected(ivalue,jvalue,0)==1)
            {
                cost=0;present=0;
                continue;
            }
        ...
        if(check_reach(jvalue)==0)
        {TV[++count]=jvalue;e--;}
        if(check_reach(ivalue)==0)
        {TV[++count]=ivalue;e--;}
        t[ivalue][jvalue]=cost;
        t[jvalue][ivalue]=cost;
        k++;
        present=cost=0;
    }
}
